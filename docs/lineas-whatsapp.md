...
El webhook identifica cada cambio por `metadata.phone_number_id`. Un identificador ausente/desconocido devuelve 503 y no se atribuye a ventas. El 200 se emite después de persistir; si falla un elemento, se procesan los demás y se devuelve 503. `whatsappMsgId` deduplica lo ya persistido en los reintentos. Las respuestas automáticas comerciales y la creación automática de leads solo se aplican a la línea comercial.
